...
portfolioRoutes.post('/', async c => {
  ...
  try {
    ...
    if (e instanceof SupabaseConnectionError) {
      return c.json(
        { success: false, error: 'supabase connection error' },
        503,
      );
    }
    const status = e instanceof HttpError ? e.statusCode : 500;
    const message =
      e instanceof Error ? e.message : '포트폴리오 생성에 실패했습니다';
...
portfolioRoutes.get('/', async c => {
  ...
  try {
    ...
    const status = e instanceof HttpError ? e.statusCode : 500;
    const message =
      e instanceof Error ? e.message : '포트폴리오 목록 조회에 실패했습니다';
    return c.json({ success: false, error: message }, status as any);
  }
});
...
portfolioRoutes.get('/:id', async c => {
  ...
  try {
    ...
    const status = e instanceof HttpError ? e.statusCode : 500;
    const message =
      e instanceof Error ? e.message : '포트폴리오 조회에 실패했습니다';
    return c.json({ success: false, error: message }, status as any);
  }
});
...
portfolioRoutes.patch('/:id', async c => {
  ...
  try {
    ...
    const status = e instanceof HttpError ? e.statusCode : 500;
    const message =
      e instanceof Error ? e.message : '포트폴리오 수정에 실패했습니다';
    return c.json({ success: false, error: message }, status as any);
  }
});
...
portfolioRoutes.delete('/:id', async c => {
  ...
  try {
    ...
    return c.json(
      { success: true, message: '포트폴리오가 삭제되었습니다' },
      200,
    );
  } catch (e) {
    if (e instanceof SupabaseConnectionError) {
    ...
    const status = e instanceof HttpError ? e.statusCode : 500;
    const message =
      e instanceof Error ? e.message : '포트폴리오 삭제에 실패했습니다';
    return c.json({ success: false, error: message }, status as any);
  }
});
